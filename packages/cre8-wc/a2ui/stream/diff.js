 ...
 */
function propDelta(prev = {}, next = {}) {
    const delta = {};
    for (const [key, value] of Object.entries(next)) {
        if (!equal(prev[key], value))
            delta[key] = value;
    }
...
function contentLists(spec) {
    const lists = new Map();
    if (spec.children)
        lists.set(null, spec.children);
    for (const [slot, list] of Object.entries(spec.slots ?? {}))
        lists.set(slot, list);
    return lists;
}
function diffList(path, slot, prev, next, ops) {
    ...
    for (let i = 0; i < shared; i++) {
        ...
        diffNode(childAt, before, after, ops);
    }
...
function diffNode(path, prev, next, ops) {
    ...
    for (const [slot] of prevLists) {
        if (!nextLists.has(slot))
            ops.push({ op: 'clear', path, slot });
    }
}
/**
