...
This project builds to the Artifactory for distribution to allow for developers on the Cre8 side to install the package.

If you're on the **Cre8** side of the firewall, add the following to your `~/.npmrc` and `~/.yarnrc` files:

```
registry "https://artifactory.pharmacy.com/artifactory/api/npm/npm-virtual/"
