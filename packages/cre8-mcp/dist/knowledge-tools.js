...
export function handleGetContentModel(input) {
    ...
    if (input.component) {
        ...
        return JSON.stringify({
            ...entry,
            rule: entry.contentVia === 'children'
    ...
    const byBucket = (bucket) => entries.filter((e) => e.contentVia === bucket).map((e) => e.component).sort();
    ...
    return JSON.stringify({
        ...
        rule: 'Every component takes content through EITHER children OR slots, never both. The split ' +
        ...
        noFreeContent: {
            ...
            note: 'Slot-only components with no "default" slot. They accept no free content in any form — ' +
                'a cre8-button label is the `text` prop, not children and not slots.default.',
        },
    }, null, 2);
}
const TOPICS = ['overview', 'content-model', 'streaming', 'events', 'validation'];
export function handleCre8Guide(input) {
    const catalog = loadA2uiCatalog();
    const entries = allEntries(catalog);
    const count = (b) => entries.filter((e) => e.contentVia === b).length;
    ...
    const sections = {
        overview: {
            whatThisIs: 'cre8 is a Lit web component design system. This connector gives you three things: the ' +
            ...
            twoWaysToEmitUi: {
                markup: 'generate_code — you produce HTML or JSX the user pastes somewhere. Right when the ' +
                    'deliverable is source.',
                liveSurface: 'ui_open_surface + ui_stream — you produce UI that appears in a browser as you go, and ' +
        ...
        'content-model': {
            rule: 'Every component takes content through EITHER children OR slots, never both. Passing the ' +
        ...
        streaming: {
            whenToUse: 'When the user should watch the UI appear, when you are building something long enough ' +
            ...
            ops: {
                ...
                appendText: '{ op: "appendText", path, text } — concatenates into the trailing text node. This is how ' +
            ...
            paths: 'Nodes are addressed by the path grammar events report back on: "$", "$.children[0]", ' +
            ...
            dataBinding: 'A prop value of { "$bind": "/pointer" } reads from the surface data model (RFC 6901). ' +
            ...
            atomicity: 'A patch either lands whole or not at all. If any op fails validation, the surface is left ' +
                'exactly as it was and the error names the path and the rule.',
        },
        events: {
            contract: 'Handlers are NAMES, never code. { "events": { "click": "upgrade-clicked" } }. You choose ' +
            ...
            reading: 'ui_events returns { component, path, event, handler, detail }. `path` is where the node ' +
                'sits right now, which is how you tell two identical buttons apart without inventing ids.',
            pattern: 'Poll with `since` set to the previous `lastSeq` so you never see an event twice, and ' +
                'waitMs to block instead of spinning.',
            whatComponentsEmit: 'Look events up per component — get_component lists them. Do not assume a component emits ' +
        ...
        validation: {
            ...
            errorShape: 'Errors are path-qualified: $.slots.body[0].props.variant: value "bogus" not in enum ' +
                '["primary", …]. A second attempt almost always succeeds because the message names both ' +
                'the location and the allowed values.',
            limits: 'A green result means "consistent with the catalog", not "correct". The catalog is ' +
...
export const knowledgeTools = [
    {
        ...
        description: 'Answers the question that breaks most generated specs: does this component take content ' +
    ...
    {
        name: 'cre8_guide',
        description: 'The briefing you would otherwise get by reading the cre8 knowledge base: how to emit UI that ' +
        ...
        inputSchema: {
            ...
            properties: {
                topic: {
                    type: 'string',
                    enum: [...TOPICS],
                    description: '"overview" (default) — what this connector does and the workflow. ' +
