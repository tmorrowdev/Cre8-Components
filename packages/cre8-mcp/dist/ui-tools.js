...
const SPEC_SHAPE = 'A2UI ComponentSpec: { component, props?, children?, slots?, events? }. Content goes through ' +
...
const OPS_SHAPE = 'PatchOp[]. Each op has an `op` and a `path` ("$" is the root, "$.children[0]" and ' +
...
export const uiTools = [
    {
        ...
        description: 'Opens a live UI surface and returns a URL a human can watch. Everything you stream into it ' +
        ...
        inputSchema: {
            ...
            properties: {
                ...
                spec: {
                    type: 'object',
                    description: 'Optional root ComponentSpec to start from. A layout container (cre8-layout-container, ' +
                ...
                theme: {
                    type: 'string',
                    description: 'Brand token sheet the viewer loads (default "cre8"). GET /themes on the same server ' +
    ...
    },
    {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                data: {
                    type: 'array',
                    description: 'DataPatch[]: { pointer, value } or { pointer, op: "remove" }. RFC 6901 pointers. ' +
                ...
                status: {
                    ...
                    description: 'Tells the viewer what you are doing. Set "done" when the UI is finished so the live ' +
    ...
    {
        name: 'ui_get_surface',
        description: 'Returns the current tree and data model of a surface, with bindings resolved. Use it to ' +
    ...
    {
        name: 'ui_events',
        description: 'Reads what the user did. Returns events fired by components you bound handlers to, each ' +
    ...
    },
    {
        ...
        description: 'Closes a surface and tells any viewer it is finished. Surfaces are also swept after an hour ' +
...
async function viewerUrl(ctx, surfaceId) {
 ...
 */
async function surfaceResource(ctx, surfaceId, title, theme) {
...
export async function handleUiTool(name, args, ctx) {
    switch (name) {
        ...
        case 'ui_stream': {
            ...
            const summary = input.status
                ? surfaceStore.setStatus(input.surfaceId, input.status, input.statusMessage)
