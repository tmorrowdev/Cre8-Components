...
export function loadVectors() {
    ...
    try {
        ...
        _vectors = JSON.parse(readFileSync(path, 'utf-8'));
    }
    catch {
        _vectors = null;
    }
...
const tokenize = (s) => new Set(s.toLowerCase().match(/[a-z]+/g) ?? []);
/**
 ...
 */
export async function rankComponents(query, fallbackTexts) {
    const vectors = loadVectors();
    const texts = vectors?.meta.texts ?? fallbackTexts;
    const queryTokens = tokenize(query);
    const lexicalScore = (id) => {
        let overlap = 0;
