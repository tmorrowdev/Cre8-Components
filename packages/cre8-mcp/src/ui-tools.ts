...
export const uiTools = [
  ...
  },
  {
    name: 'ui_stream',
    description:
      'Streams a change into an open surface. This is the only mutation tool. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        spec: {
          type: 'object',
          description:
            'The whole tree as it should now be. The server diffs it against what the surface ' +
  ...
  switch (name) {
    ...
    case 'ui_stream': {
      ...
      if (input.spec && input.ops?.length) {
        throw new Error(
          'ui_stream takes either `spec` (the whole tree, diffed for you) or `ops` (explicit ' +
            'patches), not both — the ops would be applied against a tree the diff had already moved.'
        );
      }
      if (input.spec) {
        surfaceStore.setSpec(input.surfaceId, input.spec as never);
      }
      if (input.ops?.length) {
