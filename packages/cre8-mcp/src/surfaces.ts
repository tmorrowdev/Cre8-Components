...
export class SurfaceStore {
  ...
  create(input: CreateSurfaceInput = {}): SurfaceSummary {
    ...
    if (this.surfaces.size >= this.maxSurfaces) {
      const oldest = [...this.surfaces.values()].sort((a, b) =>
  ...
  get(surfaceId: string): SurfaceRecord {
    ...
    if (!record) throw new SurfaceNotFoundError(surfaceId);
    return record;
  }
  ...
  private replayMessage(record: SurfaceRecord): SurfaceMessage {
    const { model } = record;
    return {
      v: 1,
      type: 'surface.create',
  ...
  recordEvent(
    surfaceId: string,
    event: Omit<SurfaceEventMessage, 'v' | 'type' | 'surfaceId' | 'seq'>
  ): SurfaceEventMessage {
    const record = this.get(surfaceId);
    const full: SurfaceEventMessage = {
      v: 1,
      type: 'surface.event',
      surfaceId,
      seq: ++record.eventSeq,
  ...
  eventsSince(surfaceId: string, since = 0): SurfaceEventMessage[] {
   ...
   * two calls.
   */
  awaitEvents(surfaceId: string, since = 0, timeoutMs = 25_000): Promise<SurfaceEventMessage[]> {
    const record = this.get(surfaceId);
    const pending = record.events.filter((e) => (e.seq ?? 0) > since);
    if (pending.length) return Promise.resolve(pending);
  ...
  private dispatch(record: SurfaceRecord, message: SurfaceMessage): SurfaceSummary {
  ...
  private summarize(record: SurfaceRecord): SurfaceSummary {
    return {
      surfaceId: record.surfaceId,
  ...
  private sweep(): void {
    ...
    for (const record of [...this.surfaces.values()]) {
      ...
      if (Date.parse(record.updatedAt) < cutoff) this.surfaces.delete(record.surfaceId);
    }
  }
}
