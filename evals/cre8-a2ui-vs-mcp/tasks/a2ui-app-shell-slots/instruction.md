...
The **application shell** every Innovexa page renders inside: a header region, a
main region, and a footer region, in that order, wrapped in the page layout
component.

The header carries three things:

- the Innovexa logo, linking to `/`
- the primary navigation, with the items `Overview`, `Usage`, `Billing` (mark
  `Usage` as the active item)
- the utility actions, sitting apart from the primary navigation: a `Support`
  link and a `Sign out` button

The main region contains a page header whose title is `Usage` and one section
headlined `This billing period` containing the text
`You have used 62% of your included requests.`

The footer contains the single line `(c) 2026 Innovexa, Inc.`

Place each piece of content in the region of the component that is meant to hold
it. A component only has the named slots it declares - putting content in a slot
name the component does not declare means that content never renders.
