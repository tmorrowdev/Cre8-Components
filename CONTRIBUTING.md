...
Inside the render is where the actual HTML code will be located. A structure where each individual element has a class that describes it is desirable, even if that element does not need any extra styling. Accessibility is a very big priority for customer facing Cre8 applications. Use as many native HTML elements as possible. Some important other items for accessibility:
...
- For most all other values in the CSS, we want to use CSS variables defined in our [Design Tokens Repo](https://github.com/tmorrowdev/cre8-design-tokens). If you are ever hardcoding color for example, there is a token for that. Most of your file should look like `color: var(--cre8-color-content-knockout);` except for CSS properties like `display`,`flex-direction`, `position`, visibility and afor mentioned pixel length cases.
