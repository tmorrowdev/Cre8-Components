...
stillDrifting('Six declared props never reach the DOM', () => {
  // The drift persists while any listed property is declared but never read
  // back. An agent that sets one gets silence from a spec that passes
  // validateSpec — the same shape as the cre8-heading.type bug. The authority
  // is a2ui/inert-props.json, regenerated by `pnpm audit:render`.
  const inert = json('packages/cre8-wc/a2ui/inert-props.json');
  if (!inert) return null;
  const pairs = Object.entries(inert.components ?? {}).flatMap(([tag, props]) =>
    Object.keys(props).map((prop) => [tag, prop])
  );
  if (!pairs.length) return false;
  return pairs.some(([tag, prop]) => {
    const dir = tag.replace(/^cre8-/, '');
    const src = read(`packages/cre8-wc/components/${dir}/${dir}.ts`);
    return src !== null && !new RegExp(`this\\.${prop}\\b`).test(src);
  });
});
